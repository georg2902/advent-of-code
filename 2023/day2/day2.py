...
id_pattern: Pattern = re.compile(r"(Game )((\d*))")
...
class Draw:
    ...
    def parse_draw(input: str) -> 'Draw':
        ...
        for c in colors:
            ...
            int_val = int(val)
            if str(color).startswith("b"):
                blue = int_val
            elif str(color).startswith("r"):
                red = int_val
            elif str(color).startswith("g"):
                green = int_val
            else:
                raise Exception(f"{color} unknown")
        return Draw(red, blue, green)

    @property
    def is_impossible(self):
        return validate_against_global(self.blue, self.red, self.green)


class Game:
    def __init__(self, id: str, draws: list[Draw]):
        self.id = id
        self.draws = draws
    ...
    def parse_game(line: str) -> 'Game':
        game_id = re.match(id_pattern, line.strip()).group(2)
        relevant = line.split(":")
        raw_draws = relevant[1].split(";")
        draws = [Draw.parse_draw(rd) for rd in raw_draws]
        return Game(game_id, draws)

    def validate(self):
        self.impossible = any([d.is_impossible for d in self.draws])
        #sum_blue = sum([d.blue for d in self.draws])
        #sum_red = sum([d.red for d in self.draws])
        #sum_green = sum([d.green for d in self.draws])
        #self.impossible = validate_against_global(sum_blue, sum_red, sum_green)


def parse_games(input: str) -> list[Game]:
    games = [Game.parse_game(l) for l in input.split("\n")]
    return games


def validate_full(input: str) -> int:
    games = parse_games(input)
    [g.validate() for g in games]
    return sum([int(g.id) for g in games if not g.impossible])


def validate_against_global(blue, red, green):
    return red > global_red or green > global_green or blue > global_blue


if __name__ == '__main__':
    # game = Game.parse_game("Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green")
    # assert game.id == "1"
    # game_3 = Game.parse_game("Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red")
    # game_3.validate()
    # assert game_3.impossible == True
    validated = validate_full(
